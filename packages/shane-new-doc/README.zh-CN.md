...
面向 Astro Starlight 文档站的交互式「新建文档」命令行工具。安装本包后会在你的项目里生成一个 `scripts/new-doc.js`，之后运行它即可脚手架新文档——支持交互模式，也支持一条命令直接生成。
...
这会把 `shane-new-doc` 装进你的项目并自动跑完初始化——不需要额外的 `postinstall` 步骤。如果你的终端是交互式的、又没有提前传语言参数，它只会问你**一次**：
...
> pnpm v9/v10 默认会拦截新依赖的安装脚本。执行 `pnpm approve-builds`，方向键选中 `shane-new-doc`，按空格勾选，回车确认即可。想以后都不用手动批准，可以在项目的 `package.json` 里加：
> ```json
...
📖 **https://shane-docs.pages.dev/zh-cn/cli/new-doc/**（English: **https://shane-docs.pages.dev/en/cli/new-doc/**）
